...
public class GLRecorder {
    ...
    public static void init(int w, int h, float width_ratio, float height_ratio, EGLConfig currentConfig) {
        ...
        Log.d(TAG, "init() called with: w = [" + w + "], h = [" + h + "], width_ratio = [" + width_ratio + "], height_ratio = [" + height_ratio + "], currentConfig = [" + currentConfig + "]");
...
//        }
    ...
    private static void prepareFramebuffer(int width, int height) {
        ...
        int error = GLES20.glGetError();
        Log.i(TAG, "prepareFramebuffer: "+error);
